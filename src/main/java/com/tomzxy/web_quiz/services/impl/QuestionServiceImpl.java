...
public class QuestionServiceImpl implements QuestionService {
    ...
    public void create_Question(QuestionReqDTO questionReqDTO) {
        boolean flag = questionRepo.existsByQuestionName(questionReqDTO.getQuestionName());// check if the question name
                                                                                           // is already existed
        ...
        Set<Answer> answers = answerMapper.toSetAnswer(questionReqDTO.getAnswers());// convert the answer request dto to
                                                                                    // answer
        ...
        String questionHash = questionRepo.findAllByContentHashIn(List.of(question.getContentHash())).stream()
    ...
    public QuestionResDTO update_Question(Long question_id, QuestionReqDTO questionReqDTO) {
        ...
        Set<Answer> answers = answerMapper.toSetAnswer(questionReqDTO.getAnswers()); // convert the answer request dto
                                                                                     // to answer
        question.addAnswers(answers); // save the answers to the question
    ...
    public Question findQuestion(Long question_id) {
        return questionRepo.findById(question_id)
                .orElseThrow(() -> new ApiException(AppCode.NOT_AVAILABLE, "Question not found!"));
    }
    ...
    public String generateContentHash(Question question) {
        try {
            ...
            payload.put("type", question.getType() != null ? question.getType().name() : null);
            payload.put("answerType", question.getAnswerType() != null ? question.getAnswerType().name() : ContentType.TEXT.name());
